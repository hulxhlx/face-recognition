...
data_dir='E:/anaconda/envs/tfface/Lib/site-packages/facenet/datasets/att_data'
...
lfw_pairs = 'E:/anaconda/envs/tfface/Lib/site-packages/facenet/data/pairs_att.txt'
optimizer = 'RMSPROP'
arglearning_rate=0.00001
max_nrof_epochs = 30
batch_size = 64
...
def lfwevaluate(embeddings, actual_issame, nrof_folds=10, distance_metric=0, subtract_mean=False):
	# Calculate evaluation metrics
	thresholds = np.arange(0, 4, 0.01)
	embeddings1 = embeddings[0::2]#odd and even in embedding
...
def train(arglearning_rate, epoch_size, sess, epoch, image_list, label_list, image_batch, label_batch,
	...
	dataset = loaddata.read_data_sets(data_dir)
	...
	while batch_number < epoch_size:
		...
		if batch_number % 100 == 0:
			...
			# print(sess.run(image_batch, feed_dict={image_batch :batch_features}))
			feed_dict = {learning_rate_placeholder: lr, phase_train_placeholder: True,
			             batch_size_placeholder: batch_size,image_batch :batch_features,label_batch :batch_labels }
		...
		print(
			...
			(epoch, batch_number + 1, epoch_size, duration, loss_, cross_entropy_mean_, np.sum(reg_losses_), accuracy_,
			 lr_, center_loss_))
		batch_number += 1
		train_time += duration
...
def validate(lfw_batch_size, sess, epoch, image_list, label_list, image_paths_placeholder,
             ...
             use_fixed_image_standardization,image_batch,
			             label_batch):
	...
	# sess.run( {image_paths_placeholder: image_paths_array, labels_placeholder: labels_array,
...
def evaluate(sess, lfwimg_paths_placeholder, lfwlabels_placeholder, phase_train_placeholder,
             ...
             stat, epoch, distance_metric, subtract_mean, use_flipped_images, use_fixed_image_standardization,image_batch,
			             label_batch):
	start_time = time.time()
	...
	# sess.run({lfwimg_paths_placeholder: image_paths_array, lfwlabels_placeholder: labels_array})
	...
	for i in range(nrof_batches):
		...
		for j in range(batch_size):
			img=cv2.imread(image_paths_array[j][0])
			imglist.append(img)
			labellist.append(labels_array[j][0])
		feed_dict = {phase_train_placeholder: False, batch_size_placeholder: batch_size,image_batch:imglist,label_batch:labellist}
	...
	_, _, accuracy, val, val_std, far = lfwevaluate(embeddings, actual_issame, nrof_folds=nrof_folds,
	                                                 distance_metric=distance_metric, subtract_mean=subtract_mean)
...
def save_variables_and_metagraph(sess, saver, summary_writer, model_dir, model_name, step):
	...
	saver.save(sess, checkpoint_path, global_step=step*100, write_meta_graph=False)
...
with tf.Graph().as_default():
	...
	lfwimg_paths_placeholder = tf.placeholder(tf.string, shape=(None, 1), name='lfwimage_paths')
	lfwlabels_placeholder = tf.placeholder(tf.int32, shape=(None, 1), name='lfwlabels')
	...
	with sess.as_default():
		...
		for epoch in range(1, max_nrof_epochs + 1):
			...
			if len(val_image_list) > 0 and (
				...
				validate(lfw_batch_size, sess, epoch, val_image_list, val_label_list,
			...
			t = time.time()
			if lfw_dir:
				evaluate(sess, lfwimg_paths_placeholder, lfwlabels_placeholder, phase_train_placeholder,
				         batch_size_placeholder, control_placeholder,
