...
#joint distance to analysis the performance
...
distinc = distance(embeddings1, embeddings2 )
...
disseq = distance(embeddings1, embeddings2 )

alpha = np.arange(0,1,0.01)
acc_list=[]
for i in alpha:
	dist = i*distinc+(1-i)*disseq
	thresholds = np.arange(0, 4, 0.01)
	acc_train = np.zeros((nrof_thresholds))
	for threshold_idx, threshold in enumerate(thresholds):
		tpr_, fpr_, acc_train[threshold_idx] = calculate_accuracy(threshold, dist, actual_issame)
	best_threshold_index = np.argmax(acc_train)
	tpr, fpr, accuracy = calculate_accuracy(thresholds[best_threshold_index], dist,
	                                    actual_issame)
...
dist = 0.7*distinc+(1-0.7)*disseq
np.save('bestdist.npy',dist)
nomatchdis = []
matchdis=[]
acc_train=[]
for threshold_idx, threshold in enumerate(thresholds):
